...
import org.apache.flink.runtime.taskexecutor.{TaskExecutor, TaskManagerRunner}
...
class JobManager(
  ...
  override def postStop(): Unit = {
    ...
    instanceManager.getAllRegisteredInstances.asScala.foreach {
      instance =>
        instance.getTaskManagerGateway().disconnectFromJobManager(
          instance.getId,
    ...
    if (jobGraph == null) {
      ...
      try {
        ...
        if (registerNewGraph) {
          ...
          executionGraph.setResourceID(jobGraph.getResourceID)
          log.info(s"register new job with jobid:$jobGraph")
        } else {
          //设置resourceID
          executionGraph.setResourceID(currentJobs.get(jobGraph.getJobID).getOrElse(null)._1.getResourceID);
          log.info(s"user already job's task manager:$jobGraph")
        }
